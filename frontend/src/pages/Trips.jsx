...
import { ExternalLink, Plus, Route, Trash2, Truck, X } from "lucide-react";
import { Link } from "react-router-dom";
import "./Operations.css";
import "./TripCreate.css";
import "./TripVersionControls.css";
import { getStoredTripFormVersions } from "../utils/tripFormSchema";
...
function Trips() {
  ...
  const [versions] = useState(() => {
    return getStoredTripFormVersions();
  });
  ...
  async function deleteTrip(trip) {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5087/api/trips/${encodeURIComponent(trip.id)}`, { method: "DELETE" });
      if (!response.ok && response.status !== 404) throw new Error("Trip deletion failed");
    } catch {
      const storedTrips = JSON.parse(localStorage.getItem("fleetos-copilot-trips") || "[]");
      if (!trip.id.startsWith("TR-DEMO-") && !storedTrips.some((storedTrip) => storedTrip.id === trip.id)) {
        setNotice(`${trip.id} could not be deleted. Please try again.`);
        return;
      }
    }

    const storedTrips = JSON.parse(localStorage.getItem("fleetos-copilot-trips") || "[]");
    localStorage.setItem("fleetos-copilot-trips", JSON.stringify(storedTrips.filter((storedTrip) => storedTrip.id !== trip.id)));
    setTrips((current) => current.filter((item) => item.id !== trip.id));
    window.dispatchEvent(new CustomEvent("fleetos:trip-deleted", { detail: trip }));
    setNotice(`${trip.id} deleted.`);
  }
