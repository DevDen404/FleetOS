...
function TripFormBuilder() {
  const [versions, setVersions] = useState(() => getStoredTripFormVersions());
  ...
  const addField = () => {
    const nextField = tripFormFields.find((field) => field.id === selectedFieldId);
    if (!nextField) {
      setNotice("Choose an available schema field first.");
      return;
    }
    if (active.fields.some((field) => field.id === nextField.id)) {
      setNotice(`${nextField.label} is already in this form.`);
      return;
    }
    saveVersions(versions.map((version) => version.id === active.id ? { ...version, status: "Draft", fields: [...version.fields, { ...nextField }] } : version));
    setSelectedFieldId("");
  };
  ...
  const deleteVersion = (version) => {
    if (version.version === 1) {
      setNotice("Trip intake v1 is the default form and cannot be deleted.");
      return;
    }
  ...
  return <div className="trip-builder"><div className="builder-heading"><div><p className="eyebrow">WORKSPACE CONFIGURATION</p><h1>Trip form builder</h1><p>Design and publish the form your team uses to enter trip data.</p></div><div className="builder-actions"><button className="builder-button" onClick={() => setNotice("Draft saved locally.")}><Save size={14} /> Save draft</button><button className="builder-button primary" onClick={publish}><Send size={14} /> Publish version</button></div></div>
    <div className="builder-shell"><aside className="builder-panel"><div className="builder-panel-heading"><h2>Versions</h2><button className="builder-button" onClick={createVersion}><Plus size={14} /></button></div><div className="version-list">{versions.map((version) => <div className={`version-item ${version.id === active.id ? "active" : ""}`} key={version.id}><div className="version-row"><button className="version-select" onClick={() => setActiveId(version.id)}><strong>{version.name}</strong><span>v{version.version} · {version.updated}</span><span className="version-badge">{version.status}</span></button>{version.version !== 1 && <button className="version-delete" onClick={() => deleteVersion(version)} aria-label={`Delete ${version.name}`} title="Delete version"><Trash2 size={14} /></button>}</div>{version.status === "Published" && <button className={`live-version-button ${version.id === liveId ? "live" : ""}`} onClick={() => setLive(version)}>{version.id === liveId ? "Live version" : "Set live"}</button>}</div>)}</div><p className="builder-note">Trip intake v1 is the default form and cannot be deleted. Publish a version, then choose Set live. Trips will use the live version by default.</p></aside>
      <section className="builder-panel"><div className="builder-panel-heading"><div><h2>Form fields</h2><span>{active.fields.length} of {tripFormFields.length} schema fields · {active.status}</span></div><Settings2 size={17} color="#e8a13a" /></div><div className="field-list">{active.fields.map((field) => <div className="field-config" key={field.id}><div className="field-config-top"><GripVertical size={15} className="field-grip" /><strong>{field.label}</strong><span className="field-type">{field.type}</span><button className="row-action" onClick={() => removeField(field.id)} aria-label={`Remove ${field.label}`}><Trash2 size={14} /></button></div><label className="field-toggle"><input type="checkbox" checked={field.required} disabled /> Required by trip schema</label></div>)}</div><div className="field-add-controls"><select value={selectedFieldId} onChange={(event) => setSelectedFieldId(event.target.value)} aria-label="Choose schema field to add"><option value="">Choose a schema field...</option>{tripFormFields.map((field) => <option key={field.id} value={field.id} disabled={active.fields.some((addedField) => addedField.id === field.id)}>{field.label} · {field.type}{active.fields.some((addedField) => addedField.id === field.id) ? " · Already added" : ""}</option>)}</select><button className="field-add" onClick={addField} disabled={!selectedFieldId}><Plus size={14} /> Add field</button></div>{notice && <div className="form-message">{notice}</div>}</section>
